...
docker run -it --rm --name core core
```
...
docker compose exec postgres psql -U postgres -c "CREATE DATABASE \"core\";"

# Verify connection from host (replace DB name if different)
docker compose exec postgres psql -U postgres -d core -c "SELECT 1;"

# Inspect PostgreSQL logs
docker compose logs postgres
```

### API + PostgreSQL (Compose)
...
# Image name is typically project_service, e.g. core_api
docker image tag core_api username/fastapi
...
Run from the project root. See [Alembic Setup](config/db/async-sqlalchemy-alembic.md) for full guide.
...
```shell
docker compose exec postgres psql -U postgres -d core -c "SELECT * FROM public.alembic_version ORDER BY version_num ASC;"
```
...
```shell
# Generate secrets (set in .env)
openssl rand -hex 32
```

Required: `JWT_SECRET_KEY` (signs session tokens), `SESSION_SECRET_KEY` (signs OAuth state cookie). Use one output per variable.
