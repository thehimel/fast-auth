...
Postgres creates **POSTGRES_DB** only on first initialization. If the volume was created when `.env` had a different **POSTGRES_DB** (e.g. `fastapi_db`), the current database name (e.g. `core`) may not exist.
...
Use the same value as **POSTGRES_PASSWORD** in [.env.example](../../../.env.example) (e.g. `postgres`). No leading or trailing spaces.
